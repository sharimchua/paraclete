...
const App: React.FC = () => {
  ...
  const fetchProposalsCount = async (): Promise<void> => {
    try {
      const proposals = await api.get<FrameworkProposal[]>(
        '/api/framework/proposals?status=pending'
      )
  ...
  useEffect(() => {
    ...
    const handleLinkPersona = (e: Event): void => {
      setPersonaLinkingTarget((e as CustomEvent<LinkPersonaDetail>).detail)
    }
    window.addEventListener('trigger-link-persona', handleLinkPersona as EventListener)

    const handleNavigate = (e: Event): void => {
      const { view, personId, groupId, noteId, date, messageId } = (
        e as CustomEvent<NavigateDetail>
      ).detail
    ...
    const handleThinking = (e: Event): void => {
      setIsThinking((e as CustomEvent<boolean>).detail)
    }
