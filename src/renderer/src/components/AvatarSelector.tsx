...
export const AvatarSelector: React.FC<AvatarSelectorProps> = ({ value, onChange }) => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', background: 'var(--bg-card)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', gap: '16px' }}>
                <button
                    type="button"
                    className={`btn-secondary ${mode === 'url' ? 'active' : ''}`}
                    style={{ flex: 1, background: mode === 'url' ? 'var(--primary-faded)' : 'transparent', color: mode === 'url' ? 'var(--primary)' : 'var(--text)' }}
                    onClick={() => setMode('url')}
                >
                    Custom URL
                </button>
            ...
            {mode === 'plant' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div>
                        <label style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '8px', display: 'block' }}>Color</label>
                            ...
                            {PLANT_COLORS.map(color => (
                                <button
                                    ...
                                    style={{
                                        ...
                                        borderRadius: '50%',
                                        background: color,
                                        border: currentPlantColor === color ? '3px solid var(--text)' : '2px solid transparent',
                                        cursor: 'pointer'
                                    }}
                                />
                            ))}
                    ...
                    <div>
                        <label style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '8px', display: 'block' }}>Plant Type</label>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px' }}>
                            {PLANT_TYPES.map(type => (
                                <div
                                    ...
                                    style={{
                                        cursor: 'pointer',
                                        border: currentPlantType === type ? `2px solid ${currentPlantColor}` : '2px solid transparent',
                                        borderRadius: '8px',
                                        padding: '4px',
                                        display: 'flex',
                                        alignItems: 'center',
