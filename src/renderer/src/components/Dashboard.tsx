...
const Dashboard: React.FC<Props> = ({ onSelectNote, onStartNote }) => {
  ...
  // ⚡ Bolt: Memoize expensive array aggregations and dictionary mappings
  // Prevents recalculating chart categories, colors, and maximums on every render
  const { allCategories, categoryColorMap, maxTrendCount } = useMemo(() => {
    if (!trends || trends.length === 0) {
      return { allCategories: [], categoryColorMap: {}, maxTrendCount: 1 }
    }
    const colors = [
      'var(--primary)',
      'var(--secondary)',
    ...
    return { allCategories: categories, categoryColorMap: colorMap, maxTrendCount: maxCount }
  }, [trends])
  ...
  return (
    <div
      ...
      style={{
        display: 'grid',
        gridTemplateColumns: 'minmax(300px, 1fr) minmax(300px, 1fr) minmax(280px, 0.8fr)',
        gap: '24px'
      }}
    >
          ...
          >
            ...
            <div
              style={{
                ...
                borderRadius: '8px',
                border:
                  pendingCount > 0 ? '1px solid var(--primary-faded)' : '1px solid transparent',
                cursor: 'pointer'
              }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div
          className="card"
          style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: '300px' }}
        >
          <div
            style={{
              display: 'flex',
          ...
          >
            ...
            <button
              ...
              aria-label="Add session note for this date"
            >
              <svg
                width="14"
                height="14"
            ...
            {dateMessages.length > 0 ? (
              dateMessages.map((msg) => (
                ...
                >
                  ...
                  >
                    {msg.is_inbound ? '📥' : '📤'}{' '}
                    {msg.draft_text ? msg.draft_text.substring(0, 40) + '...' : 'Untitled Message'}
                  </div>
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>
                    {msg.person
                      ? `👤 ${msg.person.name}`
