...
const MessagesList: React.FC<MessagesListProps> = ({ onSelectMessage }) => {
  ...
  // ⚡ Bolt Optimization: Wrap filteredMessages in useMemo to prevent unnecessary recalculations on re-render.
  // Hoist searchTerm.toLowerCase() outside the map/filter loop to prevent O(N) redundant string operations.
  const filteredMessages = useMemo(() => {
    const searchLower = searchTerm.toLowerCase()
    return messages.filter((m) => {
      const matchesSearch =
        (m.draft_text || '').toLowerCase().includes(searchLower) ||
        (m.person?.name || '').toLowerCase().includes(searchLower) ||
