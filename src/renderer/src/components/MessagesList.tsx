...
const MessagesList: React.FC<MessagesListProps> = ({ onSelectMessage }) => {
    ...
    const filteredMessages = messages.filter(m => {
        const matchesSearch = (m.draft_text || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return (
        <div className="messages-list-container" style={{ padding: '24px' }}>
            ...
            {isLoading ? (
                ...
                <div className="messages-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '20px' }}>
                        ...
                        >
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                                ...
                                <div style={{ 
                                    ...
                                    borderRadius: '12px', 
                                    backgroundColor: 
                                        msg.status === 'sent' ? 'rgba(74, 222, 128, 0.2)' : 
                                        msg.status === 'draft' ? 'rgba(251, 191, 36, 0.2)' : 'rgba(148, 163, 184, 0.2)',
                                    color: 
                                        msg.status === 'sent' ? '#4ade80' : 
                                        msg.status === 'draft' ? '#fbbf24' : '#94a6b8',
