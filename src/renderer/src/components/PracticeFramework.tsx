            ...
            {editingId === item.id ? (
              <div style={{ flex: 1, display: 'flex', gap: '8px' }}>
                <input
                  ...
                  autoFocus
                  onKeyDown={(e) =>
                    e.key === 'Enter' && (onUpdate(item.id, editValue), setEditingId(null))
                  }
                />
                <button
              ...
              <>
                <div
                  style={{ fontSize: '0.95rem', flex: 1, cursor: 'pointer' }}
                  onClick={() => {
          ...
          >
            <input
              ...
              onKeyDown={(e) =>
                e.key === 'Enter' &&
                newValue &&
                (onAdd(aspect, newValue), setNewValue(''), setIsAdding(false))
              }
            />
            <button
              className="btn-primary"
...
const PracticeFramework: React.FC = () => {
  ...
  const [selectedCustomRecord, setSelectedCustomRecord] = useState<CustomFrameworkRecord | null>(
    null
  )
    ...
    try {
      ...
      if (customRes.status === 'fulfilled') {
        ...
        if (selectedCustomRecord) {
          const updated = customRes.value.find(
            (r) => r.id === selectedCustomRecord.id && r.type === selectedCustomRecord.type
          )
  ...
  const handleAddCustomOverride = async (target: {
    ...
    try {
      await api.post<{ status: string; id: number }>(
        `/api/framework/custom/${target.type}/${target.id}`,
        {}
      )
      setShowAddCustom(false)
      await fetchData()
  ...
  const handleDeletePersona = async (id: number): Promise<void> => {
    setConfirmation({
      ...
      message:
        'Are you sure you want to delete this persona? All associated core rules will be unlinked (but remain in global core).',
      variant: 'danger',
      onConfirm: async () => {
        try {
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          {((activeTab === 'personas' && selectedPersonaId) ||
            ...
            <FrameworkAnalysisControls
              ...
              personId={
                selectedCustomRecord?.type === 'person' ? selectedCustomRecord.id : undefined
              }
      ...
      >
        <button
          ...
          style={{
            ...
            fontWeight: activeTab === 'core' ? 700 : 400
          }}
        >
          Global Core
        </button>
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            borderRadius: '8px 8px 0 0',
            fontWeight: activeTab === 'personas' ? 700 : 400
          }}
        >
          Personas
        </button>
        <button
          ...
          style={{
            ...
            cursor: 'pointer',
            borderRadius: '8px 8px 0 0',
            fontWeight: activeTab === 'custom' ? 700 : 400
          }}
        >
          Custom
        </button>
        <button
          ...
          style={{
            ...
            alignItems: 'center',
            gap: '8px',
            fontWeight: activeTab === 'proposals' ? 700 : 400
          }}
        >
          Proposals{' '}
          {proposals.length > 0 && (
      ...
      <div className="tab-content">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            >
              <p style={{ margin: 0, fontSize: '0.9rem' }}>
                <strong>Global Core:</strong> These settings apply to all sessions unless overridden
            ...
            {[
              {
                id: 'Tone',
                title: 'Tone & Idioms',
                desc: 'Typical vocabulary, regionalisms, and emotional tenor.'
              },
              {
                id: 'Formatting',
                title: 'Formatting & Structure',
                desc: 'Bullet points usage, greeting styles, and brevity.'
              },
              {
                id: 'Principles',
                title: 'Principles & Tenets',
                desc: 'Core philosophical anchors for communication.'
              },
              {
                id: 'Phrasing',
                title: 'Common Phrasing',
                desc: 'Specific phrases or sentence starters to use or avoid.'
              }
            ].map((section) => (
              <FrameworkAspectList
                key={section.id}
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      required
                      placeholder="e.g. Clinical Supervisor, Mentor, etc."
                    />
                  </div>
                  <div>
                    <label style={{ display: 'block', marginBottom: '8px', fontSize: '0.9rem' }}>
            ...
            >
              {personas.map((p) => (
                ...
                >
                  ...
                  >
                    {p.description ||
                      'Established professional context and baseline stylistic patterns.'}
                  </p>
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        api
                          ...
                          .then((res) => {
                            setConsolidatedView({
                              title: `${p.name} Practice Style`,
                              text: res.consolidated_text,
              ...
              {personas.length === 0 && (
                ...
                >
                  ...
                  <p>
                    Create personas to represent different professional contexts or styles of
                    practice.
                  </p>
                  <button
                    className="btn-primary"
        ...
        {activeTab === 'custom' && !selectedCustomRecord && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            >
              <p style={{ margin: 0, fontSize: '0.9rem' }}>
                <strong>Stakeholder Frameworks:</strong> Unique professional styles automatically
                extracted for specific clients or groups.
              </p>
            </div>
            ...
            >
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        {record.type === 'person' ? 'Client Override' : 'Group Override'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '8px', marginTop: 'auto' }}>
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        setSelectedCustomRecord(record)
                      }}
                    >
                      Manage
                    </button>
              ...
              {customRecords.length === 0 && (
                ...
                >
                  No stakeholder-specific frameworks yet. These will appear as the AI extracts
                  unique patterns for specific clients or groups.
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <button
                ...
                onClick={() => setSelectedCustomRecord(null)}
              >
                ← Back to List
              </button>
            ...
            >
              <p style={{ margin: 0, fontSize: '0.9rem' }}>
                <strong>Custom Overrides:</strong> These settings specifically apply when
                interacting with {selectedCustomRecord.name}.
              </p>
            </div>
              ...
              <>
                {[
                  ...
                  <FrameworkAspectList
                    ...
                    onAdd={(asp, val) =>
                      createFrameworkItem(selectedCustomRecord.framework.id, asp, val)
                    }
                    onUpdate={updateFrameworkItem}
                    onDelete={deleteFrameworkItem}
                    onMove={setMovingItem}
        ...
        {activeTab === 'personas' && selectedPersona && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            >
              <p style={{ margin: 0, fontSize: '0.9rem' }}>
                <strong>Persona Overrides:</strong> These settings override the Global Core when
                this persona is active for a session or person.
              </p>
            </div>
                ...
                {[
                  {
                    id: 'Tone',
                    title: 'Tone & Idioms',
                    desc: `Stylistic overrides for ${selectedPersona.name}.`
                  },
                  {
                    id: 'Formatting',
                    title: 'Formatting',
                    desc: `How ${selectedPersona.name} prefers to structure information.`
                  },
                  {
                    id: 'Principles',
                    title: 'Principles',
                    desc: `Specific professional values for the ${selectedPersona.name} context.`
                  },
                  {
                    id: 'Phrasing',
                    title: 'Phrasing',
                    desc: `Preferred vocabulary or stylistic markers for ${selectedPersona.name}.`
                  }
                ].map((section) => (
                  <FrameworkAspectList
                    key={section.id}
                    ...
                    onAdd={(asp, val) =>
                      createFrameworkItem(selectedPersona.framework!.id, asp, val)
                    }
                    onUpdate={updateFrameworkItem}
                    onDelete={deleteFrameworkItem}
                    onMove={setMovingItem}
                ...
                <div style={{ marginTop: '24px' }}>
                  ...
                  <div style={{ maxWidth: '400px' }}>
                    <FrameworkAnalysisControls
                      ...
                      title={`Refine ${selectedPersona.name}`}
                    />
                  </div>
                </div>
              </>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            >
              <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.9rem' }}>
                Review AI-generated suggestions to evolve your professional style. Items with higher{' '}
                <strong>Weight</strong> were observed more frequently across your practice history.
              </p>
            ...
            {proposals.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: '48px' }}>
                <p style={{ color: 'var(--text-muted)' }}>
                  No new proposals. Try running an analysis job!
                </p>
              </div>
            ) : (
              (() => {
                ...
                return [...proposals]
                  .sort((a, b) => (b.observation_count || 1) - (a.observation_count || 1))
                  .map((proposal) => {
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            >
                              ...
                              {proposal.source_owner && (
                                <span>
                                  Owner: <strong>{proposal.source_owner}</strong>
                                </span>
                              )}
                              {proposal.source_date && (
                                <span>
                                  Date: <strong>{proposal.source_date}</strong>
                                </span>
                              )}
                            </div>
                          </div>
                            ...
                            <select
                              ...
                              value={
                                proposal.is_core
                                  ? 'core'
                                  : proposal.persona_id
                                    ? proposal.persona_id.toString()
                                    : proposal.group_id
                                      ? `group-${proposal.group_id}`
                                      : proposal.person_id
                                        ? `person-${proposal.person_id}`
                                        : 'core'
                              }
                              onChange={(e) => {
                                ...
                                const updated = proposals.map((p) => {
                                  if (p.id === proposal.id) {
                                    ...
                                    if (val.startsWith('group-')) {
                                      ...
                                      const gName = p.possible_groups?.find(
                                        (g) => g.id === id
                                      )?.name
                                      return {
                                        ...p,
                            ...
                            >
                              ...
                              {proposal.persona_id && (
                                <optgroup label="Persona Framework (Recommended)">
                                  <option value={proposal.persona_id}>
                                    Persona: {proposal.persona_name || `ID ${proposal.persona_id}`}
                                  </option>
                                </optgroup>
                              )}
                              ...
                              {(proposal.group_id || proposal.person_id) && (
                                <optgroup label="Entity Frameworks (Overrides)">
                                  {proposal.group_id && (
                                    <option value={`group-${proposal.group_id}`}>
                                      Group: {proposal.group_name || `ID ${proposal.group_id}`}
                                    </option>
                                  )}
                                  {proposal.person_id && (
                                    <option value={`person-${proposal.person_id}`}>
                                      Client: {proposal.person_name || `ID ${proposal.person_id}`}
                                    </option>
                                  )}
                                </optgroup>
                              )}
      ...
      {consolidatedView && (
        <div className="modal-overlay" onClick={() => setConsolidatedView(null)}>
          ...
          >
                  ...
                  {(() => {
                    ...
                    if (levels.length === 0)
                      return (
                        ...
                        >
                          No rules defined for this context.
                        </div>
                      )
            ...
            >
              <button className="btn-primary" onClick={() => setConsolidatedView(null)}>
                Got it
              </button>
            </div>
          </div>
            ...
            <div style={{ overflowY: 'auto' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                {core && (
                  ...
                  >
                    <span>
                      🌍 <strong>Global Core</strong>
                    </span>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                      Baseline for all context
                    </span>
                  </button>
                )}
                ...
                {customRecords.map((r) => (
                  ...
                  >
                    <span>
                      {r.type === 'person' ? '👤' : '👥'} <strong>{r.name}</strong>
                    </span>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                      {r.type === 'person' ? 'Individual' : 'Group'} Custom
                    </span>
                  </button>
                ))}
              </div>
