...
def read_persons(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    # ⚡ Bolt optimization: Use selectinload for collection relationships to avoid cartesian products
    # and unexpected truncation issues during pagination or .first(). joinedload is kept for scalar relationships.
    persons = (
