...
async def load_llm_with_broadcast():
        ...
        await ws_manager.broadcast(
            {
                ...
                "data": {
                    "type": "download",
                    "prompt": f"Downloading {name}: {progress}%",
                },
            }
        )
    ...
    async def broadcast_llm_status():
        await ws_manager.broadcast(
            {"event": "llm_status", "data": llm.llm_manager.get_status()}
        )
    ...
    for filename, label in required_models:
        ...
        if not os.path.exists(path):
            sync_report(label, 0)
            success = await model_downloader.download_if_missing(
                filename, path, sync_report
            )
            if not success:
                await ws_manager.broadcast(
                    {
    ...
    await ws_manager.broadcast(
        {
            "event": "llm_start",
            "data": {"type": "warmup", "prompt": "Warming up Analysis Engine..."},
        }
    )
    try:
        ...
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "warmup", "result": "Analysis Engine Ready"},
            }
        )
...
async def create_companion_session():
    ...
    return {
        "session_id": sid,
        "url": f"http://{local_ip}:8000/static/companion/index.html?sid={sid}",
    }
...
async def get_companion_images(sid: str):
    ...
    for path in COMPANION_SESSIONS[sid]:
        ...
        images.append(
            {"filename": filename, "url": f"/static/uploads/companion/{filename}"}
        )
    return images
...
async def process_ocr(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        await ws_manager.broadcast(
            {
                ...
                "data": {
                    "type": "ocr",
                    "prompt": f"Analysing {len(temp_paths)} image(s)",
                },
            }
        )
        ...
        await ws_manager.broadcast(
            {"event": "llm_finish", "data": {"type": "ocr", "result": result}}
        )
        return {"text": result}
    finally:
    ...
    await ws_manager.broadcast(
        {
            ...
            "data": {
                "type": "ocr",
                "prompt": f"Analysing {len(image_paths)} companion image(s)",
            },
        }
    )
...
async def process_dictate(file: UploadFile = File(...)):
    await ws_manager.broadcast(
        {
            "event": "llm_start",
            "data": {"type": "dictation", "prompt": "Cleaning Audio Capture"},
        }
    )
    ...
    await ws_manager.broadcast(
        {"event": "llm_finish", "data": {"type": "dictation", "result": result}}
    )
    return {"text": result}
...
app.mount(
    "/static/companion", StaticFiles(directory=static_companion_path), name="companion"
)
...
def read_persons(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    persons = (
        ...
        .options(
            ...
            # ⚡ Bolt optimization: Use selectinload for collection relationships (groups)
            # to prevent Cartesian products and N+1 query overhead.
...
def read_person(person_id: int, db: Session = Depends(get_db)):
    db_person = (
        ...
        .options(
            joinedload(models.Person.persona).joinedload(models.Persona.framework),
            # ⚡ Bolt optimization: selectinload avoids fetching massive redundant joined rows.
            selectinload(models.Person.groups)
            .joinedload(models.Group.persona)
            .joinedload(models.Persona.framework),
        )
...
def update_person(
    person_id: int, person_update: schemas.PersonUpdate, db: Session = Depends(get_db)
):
...
def read_group(group_id: int, db: Session = Depends(get_db)):
    db_group = (
        ...
        .options(
            ...
            # ⚡ Bolt optimization: selectinload drastically speeds up collection loading like `members`,
            # avoiding an exploding Cartesian product for groups with many notes or messages.
...
async def get_session_brief(req: SessionBriefRequest, db: Session = Depends(get_db)):
    ...
    if req.person_id:
        person = (
            db.query(models.Person).filter(models.Person.id == req.person_id).first()
        )
        if person:
            person_name = person.name
            notes = (
                db.query(models.Note)
                .filter(models.Note.person_id == req.person_id)
        ...
        if group:
            ...
            notes = (
                db.query(models.Note)
                .filter(models.Note.group_id == req.group_id)
                .order_by(models.Note.date.desc())
                .limit(5)
                .all()
            )
    ...
    await ws_manager.broadcast(
        {
            "event": "llm_start",
            "data": {"type": "session_brief", "prompt": "Synthesizing Session Brief"},
        }
    )
    try:
        ...
        if req.person_id:
            ...
            reflection_query = reflection_query.filter(
                models.Reflection.person_id == req.person_id
            )
        elif req.group_id:
            topic_query = topic_query.filter(models.Topic.group_id == req.group_id)
            reflection_query = reflection_query.filter(
                models.Reflection.group_id == req.group_id
            )

        active_topics = topic_query.all()
        topics_text = (
            "\n".join([f"- {t.title}: {t.summary}" for t in active_topics])
            if active_topics
            else ""
        )

        recent_reflections = (
            reflection_query.order_by(models.Reflection.created_at.desc())
            .limit(3)
            .all()
        )
        reflections_text = (
            "\n".join([f"- {r.content}" for r in recent_reflections])
            if recent_reflections
            else ""
        )

        brief = await llm.workflows.run_session_brief(
            person_name, history_text, topics_text, reflections_text
        )
        await ws_manager.broadcast(
            {"event": "llm_finish", "data": {"type": "session_brief", "result": brief}}
        )
        return {"result": brief}
    except Exception as e:
        await ws_manager.broadcast({"event": "llm_error", "data": str(e)})
...
async def suggest_note_title(req: TitleSuggestionRequest):
    await ws_manager.broadcast(
        {
            "event": "llm_start",
            "data": {"type": "title_suggestion", "prompt": "Generating Title"},
        }
    )
    try:
        ...
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "title_suggestion", "result": title},
            }
        )
        return {"result": title}
    except Exception as e:
...
async def transient_process(req: AnalysisRequest, db: Session = Depends(get_db)):
    ...
    if person_id:
        ...
        if group:
            ...
            notes = (
                db.query(models.Note)
                .filter(models.Note.group_id == group_id)
                .order_by(models.Note.date.desc())
                .limit(5)
                .all()
            )
    ...
    framework_context = resolve_framework_items(
        db, person_id=person_id, group_id=group_id
    )

    # Taxonomy context
    existing_tags = db.query(models.Tag).all()
    tag_taxonomy = (
        ", ".join([f"{t.key}: {t.value}" for t in existing_tags])
        if existing_tags
        else "None"
    )
    ...
    await ws_manager.broadcast(
        {
            "event": "llm_start",
            "data": {"type": "note_refinement", "prompt": "Refining Session Note"},
        }
    )
    try:
        ...
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "note_refinement", "result": cleaned_text},
            }
        )
...
async def transient_extract(req: AnalysisRequest, db: Session = Depends(get_db)):
    ...
    await ws_manager.broadcast(
        {
            "event": "llm_start",
            "data": {"type": "extract_entities", "prompt": "Extracting Entities"},
        }
    )
    try:
        ...
        if req.person_id:
            p = (
                db.query(models.Person)
                .filter(models.Person.id == req.person_id)
                .first()
            )
            if p:
                entity_name = p.name
        ...
        data = await llm.workflows.run_entity_extraction(
            text=req.raw_text, context=context, grammar=grammar
        )
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "extract_entities", "result": data},
            }
        )
        return data
    except Exception as e:
...
def read_notes(
    ...
    # ⚡ Bolt optimization: Refactored multiple joinedload on collections (like tags, actions, messages)
    # into selectinload to guarantee proper limit/offset behavior and prevent huge Cartesian joins in memory.
    ...
    notes = (
        query.order_by(models.Note.date.desc(), models.Note.created_at.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def read_notes_by_date(date_str: str, db: Session = Depends(get_db)):
    return (
        db.query(models.Note)
        .filter(models.Note.date == date_str)
...
def read_note(note_id: int, db: Session = Depends(get_db)):
    # ⚡ Bolt optimization: Using selectinload on related arrays for faster lookup
    db_note = (
        db.query(models.Note)
        .options(
...
async def process_note(note_id: int, db: Session = Depends(get_db)):
    ...
    if db_note.person:
        ...
        person_name = person.name
        person_tags = ", ".join(
            [f"{t.key}: {t.value}" if t.key else t.value for t in person.tags]
        )
    ...
    # Hierarchy-aware Practise Framework
    framework_context = resolve_framework_items(
        db, person_id=db_note.person_id, group_id=db_note.group_id
    )
    ...
    await ws_manager.broadcast(
        {
            "event": "llm_start",
            "data": {"type": "clean_note", "prompt": "Expansion with Context"},
        }
    )

    try:
        cleaned_text = await llm.workflows.run_note_cleanse(
            db_note.raw_capture, context
        )
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "clean_note", "result": cleaned_text},
            }
        )
    except Exception as e:
        await ws_manager.broadcast({"event": "llm_error", "data": str(e)})
...
async def draft_note_message(note_id: int, db: Session = Depends(get_db)):
    ...
    await ws_manager.broadcast(
        {
            "event": "llm_start",
            "data": {"type": "draft_message", "prompt": "Drafting Follow-up"},
        }
    )
    ...
    try:
        ...
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "draft_message", "result": draft_text},
            }
        )
    except Exception as e:
        await ws_manager.broadcast({"event": "llm_error", "data": str(e)})
...
async def publish_note(note_id: int, db: Session = Depends(get_db)):
    ...
    await ws_manager.broadcast(
        {
            "event": "llm_start",
            "data": {"type": "publish", "prompt": "Updating Search Index"},
        }
    )
    ...
    try:
        ...
        embed_text = llm.templates.embed_note(
            title=db_note.title, text=db_note.cleaned_text or db_note.raw_capture
        )
        ...
        if embed_response:
            ...
            db_emb = (
                db.query(models.NoteEmbedding)
                .filter(models.NoteEmbedding.note_id == note_id)
                .first()
            )
            if not db_emb:
                db_emb = models.NoteEmbedding(
                    note_id=note_id, vector=json.dumps(vector)
                )
                db.add(db_emb)
            else:
        ...
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "publish", "result": "Search Index Updated"},
            }
        )
    except Exception as e:
        await ws_manager.broadcast({"event": "llm_error", "data": str(e)})
    ...
    await ws_manager.broadcast(
        {
            ...
            "data": {
                ...
                "prompt": "Searching through knowledge...",
            },
        }
    )
    try:
        ...
        if not query_embedding:
            raise HTTPException(
                status_code=500, detail="Could not generate query embedding"
            )
        ...
        if scored_notes:
            ...
            for sn in scored_notes:
                if sn["note_id"] in note_map:
                    results.append(
                        {"note": note_map[sn["note_id"]], "score": sn["score"]}
                    )
        ...
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "semantic_search", "count": len(results)},
            }
        )
        return results[:10]
    except Exception as e:
...
async def extract_note_entities(note_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        data = await llm.workflows.run_entity_extraction(
            text=db_note.raw_capture, context=context, grammar=grammar
        )
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
        ...
        await ws_manager.broadcast(
            {"event": "llm_error", "data": f"Workflow Error: {e}"}
        )
...
def link_tag(link: schemas.TagLink, db: Session = Depends(get_db)):
    ...
    if link.entity_type == "person":
        ...
        obj = (
            ...
            .filter(models.Reference.id == link.entity_id)
            .first()
        )
    else:
    ...
    try:
        ...
        for pf in data.practise_frameworks:
            ...
            for item in pf.items:
                db_pf.items.append(
                    models.PractiseFrameworkItem(**item.model_dump(exclude={"id"}))
                )
            db.add(db_pf)
            db.flush()
        ...
        for n in data.notes:
            dump = n.model_dump(
                exclude={"tags", "actions", "messages", "person", "group"}
            )
        ...
        for r in data.references:
            db_ref = models.Reference(
                **r.model_dump(exclude={"tags", "linked_notes", "persons"})
            )
            for t in r.tags:
                if t.id in tag_map:
        ...
        await ws_manager.broadcast(
            {"event": "framework_proposals_updated", "data": {"context": "import"}}
        )
        return {
            "status": "success",
            "message": "Full atomic import completed with framework entities and outreach history",
        }
    except Exception as e:
        db.rollback()
...
def get_calendar_data(db: Session = Depends(get_db)):
    note_results = (
        db.query(models.Note.date, func.count(models.Note.id).label("count"))
        .group_by(models.Note.date)
        .all()
    )

    msg_results = (
        db.query(models.Message.date, func.count(models.Message.id).label("m_count"))
        .group_by(models.Message.date)
        .all()
    )
...
def get_trends(db: Session = Depends(get_db)):
    ...
    for note in notes:
        ...
        month_data[m_key]["stacks"][stack_val] = (
