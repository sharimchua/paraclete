...
def test_get_local_ip_exception_fallback():
    ...
    with (
        patch("backend.main.socket.socket", side_effect=Exception("Method 1 fail")),
        patch(
            "backend.main.socket.gethostname", side_effect=Exception("Method 2 fail")
        ),
        patch(
            "backend.main.socket.getaddrinfo", side_effect=Exception("Method 3 fail")
        ),
    ):
