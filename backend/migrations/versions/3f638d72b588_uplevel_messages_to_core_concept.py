...
def upgrade() -> None:
    ...
    with op.batch_alter_table("messages", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column(
                "status",
                sa.Enum("DRAFT", "SENT", "ARCHIVED", name="messagestatus"),
                nullable=True,
            )
        )
        batch_op.add_column(
            sa.Column("is_inbound", sa.Boolean(), server_default="0", nullable=False)
        )
        ...
        batch_op.create_foreign_key(
            batch_op.f("fk_messages_group_id_groups"),
            "groups",
            ["group_id"],
            ["id"],
            ondelete="CASCADE",
        ...
        batch_op.create_foreign_key(
            batch_op.f("fk_messages_persona_id_personas"),
            "personas",
            ["persona_id"],
            ["id"],
        )
        batch_op.create_foreign_key(
            batch_op.f("fk_messages_person_id_persons"),
            "persons",
            ["person_id"],
            ["id"],
            ondelete="CASCADE",
