...
async def reset_framework_analysis(
    ...
    try:
        ...
        # Execute updates
        count = 0
        count += note_query.update(
            {models.Note.analyzed_for_framework: False}, synchronize_session=False
        )
        count += msg_query.update(
            {models.Message.analyzed_for_framework: False}, synchronize_session=False
        )
        count += ref_query.update(
            {models.Reference.analyzed_for_framework: False}, synchronize_session=False
        )
        ...
        await ws_manager.broadcast(
            {"event": "framework_proposals_updated", "data": {"context": "admin_reset"}}
        )
...
async def wipe_framework_data(db: Session = Depends(get_db)):
    ...
    try:
        # 1. Delete all items
        item_count = db.query(models.PractiseFrameworkItem).delete(
            synchronize_session=False
        )

        # 2. Delete all proposals
        prop_count = db.query(models.FrameworkProposal).delete(
            synchronize_session=False
        )

        # 3. Find and delete custom framework containers (not core, not persona)
        # We nullify links first to avoid foreign key issues
        db.query(models.Person).update(
            {models.Person.custom_framework_id: None}, synchronize_session=False
        )
        db.query(models.Group).update(
            {models.Group.custom_framework_id: None}, synchronize_session=False
        )

        # Now delete all frameworks that are NOT core and NOT used by a persona
        persona_fw_ids = [
            p.framework_id for p in db.query(models.Persona).all() if p.framework_id
        ]
        fw_to_delete = (
        ...
        db.commit()
        await ws_manager.broadcast(
            {"event": "framework_proposals_updated", "data": {"context": "admin_wipe"}}
        )
        return {
            "status": "success",
