...
async def get_framework_audit(
    persona_id: Optional[int] = None, db: Session = Depends(get_db)
):
    # 1. Fetch hierarchy
    core_fw = (
    ...
    core_text = (
        "\n".join([f"- [{i.aspect}]: {i.value}" for i in core_fw.items])
        if core_fw
        else "None"
    )
    ...
    if persona_id:
        ...
        if persona:
            ...
            if persona.framework:
                persona_text = "\n".join(
                    [f"- [{i.aspect}]: {i.value}" for i in persona.framework.items]
                )
    ...
    await ws_manager.broadcast(
        {
            ...
            "data": {
                ...
                "prompt": "Auditing Practice Framework",
            },
        }
    )
    ...
    try:
        ...
        await ws_manager.broadcast(
            {"event": "llm_finish", "data": {"type": "framework_audit"}}
        )
...
async def trigger_framework_analysis(
    ...
    msg_query = db.query(models.Message.id).filter(
        or_(
            models.Message.analyzed_for_framework == False,
            models.Message.analyzed_for_framework == None,
        )
    )
    ref_query = db.query(models.Reference.id).filter(
        or_(
    ...
    for (nid,) in note_query.all():
        jid = background_manager.add_job(
            f"Analyse Note {nid}",
            worker_wrapper,
            item_type="note",
            item_id=nid,
    ...
    await ws_manager.broadcast(
        {"event": "framework_proposals_updated", "data": {"reason": "analysis_started"}}
    )
...
def stitch_framework(db_fw: models.PractiseFramework):
    ...
    by_aspect = {"tone": [], "formatting": [], "phrasing": [], "principles": []}
    ...
    map = {
        ...
        "phrasing": "phrasing",
        "common phrasing": "phrasing",
        "principles": "principles",
        "principles & tenets": "principles",
        "tenets": "principles",
    }
...
def parse_and_sync_items(
    db: Session,
    db_fw: models.PractiseFramework,
    framework_data: schemas.PractiseFrameworkCreate,
):
        ...
        for val in lines:
            if val not in existing_items:
                new_item = models.PractiseFrameworkItem(
                    framework_id=db_fw.id, aspect=aspect, value=val
                )
                db.add(new_item)
            else:
...
def update_core_framework(
    framework: schemas.PractiseFrameworkCreate, db: Session = Depends(get_db)
):
    core = (
        db.query(models.PractiseFramework)
        .filter(models.PractiseFramework.is_core == True)
...
def update_framework(
    framework_id: int,
    framework: schemas.PractiseFrameworkCreate,
    db: Session = Depends(get_db),
):
    db_framework = (
        db.query(models.PractiseFramework)
        .filter(models.PractiseFramework.id == framework_id)
        .first()
    )
    if not db_framework:
    ...
    if not db_framework.items and not db_framework.is_core:
        ...
        persona_link = (
            ...
            .filter(models.Persona.framework_id == db_framework.id)
            .first()
        )
        if not persona_link:
            # It's a custom Person/Group framework. If empty, wipe the container.
            db.query(models.Person).filter(
                models.Person.custom_framework_id == db_framework.id
            ).update({"custom_framework_id": None})
            db.query(models.Group).filter(
                models.Group.custom_framework_id == db_framework.id
            ).update({"custom_framework_id": None})
            ...
            return {
                ...
                "items": [],
                "name": "Deleted",
            }  # Return a dummy for the response model then refresh
...
def delete_framework(framework_id: int, db: Session = Depends(get_db)):
    ...
    db.query(models.Person).filter(
        models.Person.custom_framework_id == framework_id
    ).update({"custom_framework_id": None})
    db.query(models.Group).filter(
        models.Group.custom_framework_id == framework_id
    ).update({"custom_framework_id": None})
    db.query(models.Persona).filter(models.Persona.framework_id == framework_id).update(
        {"framework_id": None}
    )
...
def create_persona(persona: schemas.PersonaCreate, db: Session = Depends(get_db)):
    ...
    if not persona.framework_id:
        db_framework = models.PractiseFramework(
            name=f"{persona.name} Framework", is_core=False
        )
        db.add(db_framework)
        db.commit()
        db.refresh(db_framework)
...
def read_proposals(status: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    if status:
        ...
        if s in models.FrameworkProposalStatus.__members__:
            query = query.filter(
                models.FrameworkProposal.status == models.FrameworkProposalStatus[s]
            )
        else:
    ...
    for p in proposals:
        ...
        if p.source_type == "Note":
            source_note = (
                db.query(models.Note).filter(models.Note.id == p.source_id).first()
            )
            if source_note:
            ...
            msg = (
                ...
                .filter(models.Message.id == p.source_id)
                .first()
            )
            if msg and msg.note:
            ...
            ref = (
                ...
                .filter(models.Reference.id == p.source_id)
                .first()
            )
            if ref:
        ...
        if source_note:
            p_out.source_date = (
                source_note.date.strftime("%Y-%m-%d") if source_note.date else "Unknown"
            )
            ...
            if source_note.person:
                ...
                if source_note.person.groups:
                    p_out.possible_groups = [
                        schemas.GroupBadge.model_validate(g)
                        for g in source_note.person.groups
                    ]
                ...
                if source_note.person.persona_id and not p_out.persona_id:
                    ...
                    p_out.persona_name = (
                        source_note.person.persona.name
                        if source_note.person.persona
                        else f"Persona {p_out.persona_id}"
                    )
            ...
            # Potential target: The Group (if note is directly against a group)
                ...
                if source_note.group.persona_id and not p_out.persona_id:
                    ...
                    p_out.persona_name = (
                        source_note.group.persona.name
                        if source_note.group.persona
                        else f"Persona {p_out.persona_id}"
                    )
            else:
        ...
        if p_out.persona_id and not p_out.persona_name:
            persona = (
                db.query(models.Persona)
                .filter(models.Persona.id == p_out.persona_id)
                .first()
            )
            if persona:
                p_out.persona_name = persona.name
        if p_out.group_id and not p_out.group_name:
            group = (
                db.query(models.Group).filter(models.Group.id == p_out.group_id).first()
            )
            if group:
                p_out.group_name = group.name
        if p_out.person_id and not p_out.person_name:
            person = (
                db.query(models.Person)
                .filter(models.Person.id == p_out.person_id)
                .first()
            )
            if person:
                p_out.person_name = person.name
...
async def resolve_proposal(
    proposal_id: int, resolution: ProposalResolution, db: Session = Depends(get_db)
):
    approved = resolution.approved
    proposal = (
        db.query(models.FrameworkProposal)
        .filter(models.FrameworkProposal.id == proposal_id)
        .first()
    )
    if not proposal:
    ...
    if approved:
        ...
        effective_is_core = (
            resolution.override_is_core
            if resolution.override_is_core is not None
            else proposal.is_core
        )
        effective_persona_id = (
            resolution.override_persona_id
            if resolution.override_persona_id is not None
            else proposal.persona_id
        )
        effective_person_id = (
            resolution.override_person_id
            if resolution.override_person_id is not None
            else proposal.person_id
        )
        effective_group_id = (
            resolution.override_group_id
            if resolution.override_group_id is not None
            else proposal.group_id
        )
        ...
        if effective_person_id:
            person = (
                db.query(models.Person)
                .filter(models.Person.id == effective_person_id)
                .first()
            )
            if person:
                if not person.custom_framework_id:
                    person.custom_framework = models.PractiseFramework(
                        name=f"{person.name} Custom", is_core=False
                    )
                    db.commit()
            ...
            group = (
                ...
                .filter(models.Group.id == effective_group_id)
                .first()
            )
            if group:
                if not group.custom_framework_id:
                    group.custom_framework = models.PractiseFramework(
                        name=f"{group.name} Custom", is_core=False
                    )
                    db.commit()
            ...
            persona = (
                db.query(models.Persona)
                .filter(models.Persona.id == effective_persona_id)
                .first()
            )
            if persona:
                target_framework = (
                    db.query(models.PractiseFramework)
                    .filter(models.PractiseFramework.id == persona.framework_id)
                    .first()
                )
...
async def reject_all_pending_proposals(db: Session = Depends(get_db)):
    ...
    ).update({models.FrameworkProposal.status: models.FrameworkProposalStatus.REJECTED})
    db.commit()
    from ..websockets_manager import ws_manager

    await ws_manager.broadcast(
        {"event": "framework_proposals_updated", "data": {"reason": "all_rejected"}}
    )
    return {"status": "success"}
...
def read_custom_frameworks(db: Session = Depends(get_db)):
    ...
    persons = (
        db.query(models.Person).filter(models.Person.custom_framework_id != None).all()
    )
    groups = (
        db.query(models.Group).filter(models.Group.custom_framework_id != None).all()
    )
...
async def get_consolidated_framework(
    entity_type: str, entity_id: int, db: Session = Depends(get_db)
):
    ...
    # We want a structured response, not just text
    from ..services.framework_resolver import (
        resolve_framework_items,
        get_resolved_framework_data,
    )

    text = resolve_framework_items(
        db, persona_id=persona_id, person_id=person_id, group_id=group_id
    )
    structured_data = get_resolved_framework_data(
        db, persona_id=persona_id, person_id=person_id, group_id=group_id
    )

    return {"consolidated_text": text, "structured_data": structured_data}
...
async def synthesize_all_proposals(db: Session = Depends(get_db)):
    ...
    for p in proposals:
        prop_list.append(
            f"ID: {p.id} | Aspect: {p.aspect} | Action: {p.action} | Value: {p.value} | Scope: {'Core' if p.is_core else f'Persona {p.persona_id}'}"
        )
    ...
    await ws_manager.broadcast(
        {
            ...
            "data": {
                ...
                "prompt": f"Synthesizing {len(proposals)} proposals",
            },
        }
    )
    ...
    try:
        ...
        if start_idx == -1 or end_idx == 0:
            print(
                f"DEBUG: LLM response did not contain JSON. Content: {result_text[:500]}..."
            )
            raise HTTPException(
                status_code=500,
                detail="LLM response did not contain a valid JSON object.",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to parse LLM synthesis JSON: {je}"
            )

        new_proposals = data.get("proposals", [])
        print(
            f"DEBUG: Synthesis complete. Generated {len(new_proposals)} merged proposals."
        )
        ...
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "framework_synthesis", "result": "Synthesis Complete"},
            }
        )
...
def link_persona_to_entity(link: PersonaLink, db: Session = Depends(get_db)):
    if link.entity_type == "person":
        person = (
            db.query(models.Person).filter(models.Person.id == link.entity_id).first()
        )
        if not person:
...
def link_persona_to_person(
    person_id: int, persona_id: int, db: Session = Depends(get_db)
):
...
def link_persona_to_group(
    group_id: int, persona_id: int, db: Session = Depends(get_db)
):
...
async def draft_persona_message(
    note_id: int, persona_id: Optional[int] = None, db: Session = Depends(get_db)
):
    ...
    if not effective_persona_id:
        if note.person:
            ...
            if note.person.tags:
                overrides.append(
                    f"Individual Overrides ({note.person.name}): "
                    + ", ".join([f"{t.key}: {t.value}" for t in note.person.tags])
                )
            ...
            if not effective_persona_id and note.person.groups:
                for g in note.person.groups:
                    if g.persona_id:
                        ...
                        overrides.append(
                            f"Inherited Group Context ({g.name}): {g.description or ''}"
                        )
                        break
        elif note.group:
            ...
            if note.group.description:
                overrides.append(
                    f"Group Context ({note.group.name}): {note.group.description}"
                )

    if not effective_persona_id:
        raise HTTPException(
            status_code=400,
            detail="No professional persona linked or inherited for this session.",
        )
    ...
    await ws_manager.broadcast(
        {
            ...
            "data": {
                "type": "persona_draft",
                "prompt": f"Drafting message for {entity_name}",
            },
        }
    )
    ...
    try:
        ...
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "persona_draft", "result": "Draft Complete"},
            }
        )
        return {
            "draft": draft,
            ...
            "is_inherited": (
                persona_id is None
                and effective_persona_id
                != (note.person.persona_id if note.person else None)
            ),
        }
    except Exception as e:
        await ws_manager.broadcast({"event": "llm_error", "data": str(e)})
...
async def move_framework_item(
    item_id: int, request: MoveItemRequest, db: Session = Depends(get_db)
):
    target_type = request.target_type
    target_id = request.target_id
    item = (
        db.query(models.PractiseFrameworkItem)
        .filter(models.PractiseFrameworkItem.id == item_id)
        .first()
    )
    if not item:
    ...
    if target_type == "core":
        ...
        persona = (
            db.query(models.Persona).filter(models.Persona.id == target_id).first()
        )
        if persona:
            target_framework = persona.framework
...
def create_framework_item(
    framework_id: int,
    item: schemas.PractiseFrameworkItemBase,
    db: Session = Depends(get_db),
):
    new_item = models.PractiseFrameworkItem(
        framework_id=framework_id, aspect=item.aspect, value=item.value
    )
    db.add(new_item)
    db.commit()
    db.refresh(new_item)
...
def update_framework_item(
    item_id: int,
    item_update: schemas.PractiseFrameworkItemBase,
    db: Session = Depends(get_db),
):
    item = (
        db.query(models.PractiseFrameworkItem)
