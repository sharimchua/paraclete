...
async def get_reference_proposals(
    ...
    query = db.query(models.ReferenceProposal).filter(
        models.ReferenceProposal.status == status_enum
    )
    if note_id:
    ...
    proposals = query.all()
    print(
        f"FORENSIC: Fetched {len(proposals)} proposals for note_id={note_id}, status={status}"
    )
    return proposals
...
async def suggest_references_endpoint(
    req: schemas.ReferenceSuggestionRequest = Depends(), db: Session = Depends(get_db)
):
    return await suggest_references(db=db, req=req)


@router.get("/", response_model=List[schemas.Reference])
def read_references(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    # Performance Optimization: Use selectinload to eagerly load tags and prevent N+1 queries during Pydantic serialization
    return (
        db.query(models.Reference)
        .options(selectinload(models.Reference.tags))
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def read_reference(reference_id: int, db: Session = Depends(get_db)):
    ...
    db_ref = (
        db.query(models.Reference)
        .options(selectinload(models.Reference.tags))
        .filter(models.Reference.id == reference_id)
        .first()
    )
    if db_ref is None:
...
async def update_reference(
    reference_id: int, reference: schemas.ReferenceCreate, db: Session = Depends(get_db)
):
    db_ref = (
        db.query(models.Reference).filter(models.Reference.id == reference_id).first()
    )
    if db_ref is None:
        raise HTTPException(status_code=404, detail="Reference not found")
...
async def extract_references_task(note_id: int):
    ...
    try:
        ...
        await ws_manager.broadcast(
            {
                ...
                "data": {
                    ...
                    "prompt": "Extracting Professional Concepts",
                },
            }
        )
        ...
        existing_proposals = (
            db.query(models.ReferenceProposal)
            .filter(models.ReferenceProposal.source_note_id == note_id)
            .all()
        )

        # Performance Optimization: Pre-compute sets of existing titles for O(1) membership testing
        # instead of O(N^2) any() calls inside the loop.
        existing_ref_titles = {r.title.lower() for r in existing_refs if r.title}
        existing_prop_titles = {p.title.lower() for p in existing_proposals if p.title}
        ...
        for prop in raw_proposals:
            ...
            if title_lower in existing_ref_titles:
                print(
                    f"FORENSIC SKIP: '{prop['title']}' already exists in Reference Library."
                )
                continue
            if title_lower in existing_prop_titles:
                print(
                    f"FORENSIC SKIP: '{prop['title']}' already exists as a Proposal for this note."
                )
                continue
        ...
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "reference_extraction", "count": added_count},
            }
        )
...
async def extract_references_from_note(note_id: int, db: Session = Depends(get_db)):
    ...
    if any(
        j["name"] == job_name and j["status"] in ["pending", "running"]
        for j in active_jobs
    ):
        raise HTTPException(
            status_code=400, detail="Extraction already in progress for this note."
        )
...
async def accept_reference_proposal(proposal_id: int, db: Session = Depends(get_db)):
    proposal = (
        db.query(models.ReferenceProposal)
        .filter(models.ReferenceProposal.id == proposal_id)
        .first()
    )
    if not proposal:
    ...
    note_ref = models.note_references.insert().values(
        note_id=proposal.source_note_id, reference_id=new_ref.id
    )
    db.execute(note_ref)
...
async def suggest_references(db: Session, req: schemas.ReferenceSuggestionRequest):
    """
    ...
    if req.note_id:
        ...
        if note:
            ...
            if not effective_query:
                effective_query = (
                    f"{note.title} {note.cleaned_text or note.raw_capture or ''}"
                )
    if req.person_id:
        person = (
    ...
    await ws_manager.broadcast(
        {
            ...
            "data": {
                ...
                "prompt": "Finding relevant references",
            },
        }
    )
    ...
    try:
        ...
        if not query_embedding_resp:
            await ws_manager.broadcast(
                {"event": "llm_finish", "data": {"type": "reference_suggestion"}}
            )
            return []
        ...
        # Performance Optimization: Use selectinload to eagerly load tags and embedding to prevent N+1 queries during iteration
        all_refs = (
            db.query(models.Reference)
            .options(
        ...
        results = [r for score, r in scored_refs[: req.limit]]
        await ws_manager.broadcast(
            {
                "event": "llm_finish",
                "data": {"type": "reference_suggestion", "count": len(results)},
            }
        )
        return results
    except Exception as e:
    ...
    try:
        ...
        if embed_response:
            ...
            if db_ref.embedding:
                ...
                db_emb = models.ReferenceEmbedding(
                    reference_id=db_ref.id, vector=json.dumps(vector)
                )
                db.add(db_emb)
