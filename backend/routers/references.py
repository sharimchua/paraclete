...
def _sync_get_suggest_context(
    db: Session, req: schemas.ReferenceSuggestionRequest
) -> Tuple[Set[int], Optional[str], List[models.Reference]]:
    context_tag_ids = set()
    ...
    if req.note_id:
        note = (
            ...
            .filter(models.Note.id == req.note_id)
            .first()
        )
        if note:
    ...
    if req.person_id:
        person = (
            ...
            .options(selectinload(models.Person.tags))
    ...
    if req.group_id:
        group = (
            ...
            .options(selectinload(models.Group.tags))
            .filter(models.Group.id == req.group_id)
            .first()
        )
        if group:
    ...
    if not effective_query:
        return context_tag_ids, effective_query, []
    ...
    return context_tag_ids, effective_query, all_refs


def _sync_score_references(
    all_refs: List[models.Reference],
    q_vec: np.ndarray,
    norm_q: float,
    context_tag_ids: Set[int],
    limit: int,
...
async def suggest_references(db: Session, req: schemas.ReferenceSuggestionRequest):
    ...
    """
    context_tag_ids, effective_query, all_refs = await asyncio.to_thread(
        _sync_get_suggest_context, db, req
    )
    ...
    try:
        ...
        results = await asyncio.to_thread(
            _sync_score_references, all_refs, q_vec, norm_q, context_tag_ids, req.limit
        )
