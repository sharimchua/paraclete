...
def create_reflection(
    reflection: schemas.ReflectionCreate, db: Session = Depends(get_db)
):
...
def get_reflections(
    ...
    return (
        query.order_by(models.Reflection.created_at.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def update_reflection(
    ...
    db_reflection = (
        ...
        .filter(models.Reflection.id == reflection_id)
        .first()
    )
