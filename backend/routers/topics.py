...
def create_topic(topic: schemas.TopicCreate, db: Session = Depends(get_db)):
    ...
    if topic.source_note_id:
        note = (
            db.query(models.Note).filter(models.Note.id == topic.source_note_id).first()
        )
        if note:
            db_topic.notes.append(note)
    if topic.source_message_id:
        msg = (
            db.query(models.Message)
            .filter(models.Message.id == topic.source_message_id)
            .first()
        )
        if msg:
            db_topic.messages.append(msg)
    if topic.source_reflection_id:
        ref = (
            db.query(models.Reflection)
            .filter(models.Reflection.id == topic.source_reflection_id)
            .first()
        )
        if ref:
            db_topic.reflections.append(ref)
...
def get_topics(
    ...
    # ⚡ Bolt: Fixed N+1 query problem by eagerly loading related collections
    # This prevents the `enrich_topic` helper from firing a separate database query
    # for each relation of every returned topic during Pydantic serialization.
    ...
    topics = (
        query.order_by(models.Topic.updated_at.desc()).offset(skip).limit(limit).all()
    )
...
def update_topic(
    ...
    if "state" in update_data and update_data["state"] != db_topic.state:
        if (
            update_data["state"] == models.TopicState.active
            and db_topic.state == models.TopicState.future
        ):
...
async def summarize_topic_background(topic_id: int):
    ...
    db = SessionLocal()
    try:
        # ⚡ Bolt: Use eager loading to resolve N+1 overhead when building text_parts
        topic = (
        ...
        for n in topic.notes:
            if n.cleaned_text:
                text_parts.append(f"Note: {n.cleaned_text}")
        for m in topic.messages:
            if m.sent_text:
                text_parts.append(f"Message: {m.sent_text}")
            elif m.draft_text:
                text_parts.append(f"Message Draft: {m.draft_text}")
...
def link_to_topic(
    ...
    if entity_type == "note":
        ...
        if item and item not in db_topic.notes:
            db_topic.notes.append(item)
    elif entity_type == "message":
        item = db.query(models.Message).filter(models.Message.id == entity_id).first()
        if item and item not in db_topic.messages:
            db_topic.messages.append(item)
    elif entity_type == "reflection":
        item = (
            db.query(models.Reflection)
            .filter(models.Reflection.id == entity_id)
            .first()
        )
        if item and item not in db_topic.reflections:
            db_topic.reflections.append(item)
    else:
...
async def suggest_topic(req: TopicSuggestionRequest, db: Session = Depends(get_db)):
    ...
    if req.entity_type == "note":
        ...
        item = (
            db.query(models.Message).filter(models.Message.id == req.entity_id).first()
        )
        if not item:
        ...
        item = (
            db.query(models.Reflection)
            .filter(models.Reflection.id == req.entity_id)
            .first()
        )
        if not item:
    ...
    query = db.query(models.Topic).filter(
    ...
    topics_str = "\n".join(
        [f"ID {t.id}: {t.title} - {t.summary}" for t in active_topics]
    )
