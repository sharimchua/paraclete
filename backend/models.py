...
class Person(Base):
    ...
    custom_framework_id = Column(
        Integer, ForeignKey("practise_frameworks.id"), nullable=True
    )
    ...
    reflections = relationship(
        "Reflection", back_populates="person", cascade="all, delete-orphan"
    )
    topics = relationship(
        "Topic", back_populates="person", cascade="all, delete-orphan"
    )
    references = relationship(
        "Reference", secondary=person_references, back_populates="persons"
    )
...
class Group(Base):
    ...
    reflections = relationship(
        "Reflection", back_populates="group", cascade="all, delete-orphan"
    )
...
class Note(Base):
    ...
    actions = relationship(
        "Action", back_populates="note", cascade="all, delete-orphan"
    )
    messages = relationship(
        "Message", back_populates="note", cascade="all, delete-orphan"
    )
    references = relationship(
        "Reference", secondary=note_references, back_populates="linked_notes"
    )
    generated_references = relationship(
        "Reference", back_populates="source_note", cascade="all, delete-orphan"
    )
...
class Reference(Base):
    ...
    linked_notes = relationship(
        "Note", secondary=note_references, back_populates="references"
    )
    persons = relationship(
        "Person", secondary=person_references, back_populates="references"
    )
    embedding = relationship(
        "ReferenceEmbedding",
...
class PractiseFrameworkItem(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    framework_id = Column(
        Integer, ForeignKey("practise_frameworks.id", ondelete="CASCADE")
    )
...
class FrameworkProposal(Base):
    ...
    is_core = Column(Boolean, default=False)
    status = Column(
        Enum(FrameworkProposalStatus), default=FrameworkProposalStatus.PENDING
    )
    created_at = Column(DateTime, default=datetime.utcnow)
...
class Reflection(Base):
    ...
    topics = relationship(
        "Topic", secondary=topic_reflections, back_populates="reflections"
    )
...
class Topic(Base):
    ...
    source_reflection_id = Column(
        Integer, ForeignKey("reflections.id", ondelete="SET NULL"), nullable=True
    )
    ...
    messages = relationship(
        "Message", secondary=topic_messages, back_populates="topics"
    )
    reflections = relationship(
        "Reflection", secondary=topic_reflections, back_populates="topics"
    )
