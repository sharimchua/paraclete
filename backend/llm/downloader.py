...
class ModelDownloader:
    ...
    async def download_if_missing(self, model_name: str, target_path: str, progress_callback: Optional[Callable[[str, int], None]] = None):
        """
        ...
        if os.path.exists(target_path) and os.path.getsize(target_path) > 100 * 1024 * 1024:
            return True
        return await asyncio.to_thread(self.download_if_missing_sync, model_name, target_path, progress_callback)

    def download_if_missing_sync(self, model_name: str, target_path: str, progress_callback: Optional[Callable[[str, int], None]] = None):
        """
    ...
    def _sync_download(self, model_name: str, url: str, target_path: str, progress_callback: Optional[Callable[[str, int], None]]):
