...
class LLMManager:
    ...
    def load_model(
        ...
        with self.lock:
            ...
            if (
                ...
                and self.active_config.get("path") == model_path
                and self.active_config.get("n_ctx") == n_ctx
            ):
                return

            # Use default if path not provided
            path = (
                model_path or self.model_path or self._get_default_model_path(use_case)
            )
            ...
            print(
                f"DEBUG: Loading model from {path} (n_ctx={n_ctx}, use_case={use_case})..."
            )
            try:
                ...
                if "moe" in path.lower() and os.path.exists(mmproj_path):
                    print(
                        f"DEBUG: Found Vision Projector. Initializing Multimodal Handler..."
                    )
                    try:
    ...
    def ensure_model(self, use_case="analysis"):
        ...
        if not os.path.exists(path):
            print(
                f"DEBUG: Required model {actual_filename} missing. Triggering download..."
            )
    ...
    def get_status(self):
        ...
        return {
            ...
            "model_name": (
                os.path.basename(self.active_config.get("path", ""))
                if self.active_config.get("path")
                else "None"
            ),
        }
    ...
    def generate(
        ...
        **kwargs,
        ...
        with self.lock:
            return self.model(prompt, grammar=grammar, stream=stream, **kwargs)
    ...
    def call(
        ...
        **kwargs,
        ...
        stop = kwargs.pop(
            "stop", ["<turn|>", "<|channel|>", "<eos>", "(Note:", "Note:"]
        )
        ...
        if lines:
            ...
            if len(lines[0]) < 120 and any(
                kw in first_line for kw in preamble_keywords
            ):
        ...
        if content.startswith("#### SESSION FOCUS: #### SESSION FOCUS:"):
            content = content.replace(
                "#### SESSION FOCUS: #### SESSION FOCUS:", "#### SESSION FOCUS:", 1
            )
