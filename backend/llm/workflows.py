...
async def run_note_cleanse(text: str, context: dict) -> str:
    ...
    prompt = templates.clean_session_note(
        ...
        practitioner_preferred_name=context.get(
...
async def run_entity_extraction(
    text: str, context: str = "", grammar: str = None
) -> dict:
...
async def run_draft_message(context: dict) -> str:
    ...
    return await llm_manager.acall(prompt=prompt, system=system_prompt, max_tokens=1024)


async def run_persona_draft(
    note_content: str, persona_name: str, persona_bio: str, references: str
) -> str:
...
async def run_dictation(audio_filename: str) -> str:
    ...
    return await llm_manager.acall(
        prompt=prompt, system="Transcribe and clean up audio content.", max_tokens=1024
    )


async def run_session_brief(
    person_name: str,
    previous_notes: str,
    active_topics: str = "",
    ...
    prompt = templates.session_brief(
...
async def run_reformat(
    selected_text: str, user_prompt: str, full_context: str, framework_context: str = ""
) -> str:
    ...
    try:
        ...
        if match:
            ...
            try:
                ...
                if isinstance(data, list):
                    print(
                        f"FORENSIC: Successfully parsed array via Match 1. Count: {len(data)}"
                    )
                    return data
            except Exception as e:
        ...
        if "[" in json_str and "]" in json_str:
            ...
            try:
                ...
                if isinstance(data, list):
                    print(
                        f"FORENSIC: Successfully parsed array via bracket matching. Count: {len(data)}"
                    )
                    return data
            except Exception as e:
...
async def run_topic_summary(topic_title: str, topic_content: str) -> str:
    ...
    prompt = templates.topic_summary(
